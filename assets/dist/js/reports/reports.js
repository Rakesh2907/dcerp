			...
			success: function(result){
				var res = JSON.parse(result);
				 if(res.status == 'success'){
				 		export_inward_excel_sheet(from_date,to_date);
				 }else{
				 	swal({
							title: "",
						  	text: res.message,
...
function export_inward_excel_sheet(from_date,to_date){
	$.ajax({
	 	 	type: "POST",
	 	 	url: baseURL +"Commonrequesthandler_ui/export_inward_excel_sheet",
	 	 	headers: { 'Authorization': user_token },
	 	 	cache: false,
			...
			success: function(result){
				setTimeout(function(){
		                  ...
		                  window.open(baseURL +'Commonrequesthandler_ui/export_inward_excel_sheet/?from_date='+from_date+'&to_date='+to_date,'_blank' );
		        }, 4000); 
			}
	 });
}
